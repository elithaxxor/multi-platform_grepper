...
void system_information()
{
    ...
    // char *win_ver = NULL, *mac_ver = NULL, *mac_build = NULL, *mac_name = NULL, *mac_host = NULL, *mac_user = NULL, *mac_model = NULL,
    //      *mac_serial = NULL, *mac_uuid = NULL, *mac_arch = NULL, *mac_kernel = NULL, *mac_uptime = NULL, *mac_load = NULL,
    //      *mac_cpu = NULL, *mac_mem = NULL, *mac_swap = NULL, *mac_disk = NULL, *mac_temp = NULL, *mac_battery = NULL,
    //      *mac_network = NULL, *mac_gateway = NULL, *linux_ver = NULL, *linux_host = NULL, *linux_user = NULL,
    //      *linux_model = NULL, *linux_arch = NULL, *linux_kernel = NULL, *linux_uptime = NULL, *linux_load = NULL,
    //      *linux_cpu = NULL, *linux_mem = NULL, *linux_swap = NULL, *linux_disk = NULL, *linux_temp = NULL,
    //      *linux_battery = NULL, *linux_network = NULL, *linux_gateway = NULL;

    char *win_ver;
    char *mac_ver; char *mac_build; char *mac_name; char *mac_host; char *mac_user; char *mac_model;
         char *mac_serial; char *mac_uuid; char *mac_arch; char *mac_kernel; char *mac_uptime; char *mac_load;
         char *mac_cpu; char *mac_mem; char *mac_swap; char *mac_disk; char *mac_temp; char *mac_battery;
         char *mac_network; char *mac_gateway; char *linux_ver; char *linux_host; char *linux_user;
        char *linux_model; char *linux_arch; char *linux_kernel; char *linux_uptime; char *linux_load; char
         *linux_cpu; char *linux_mem; char *linux_swap; char *linux_disk; char *linux_temp; char
         *linux_battery; char *linux_network; char *linux_gateway;
...
#endif
    // Assume Linux (or some other POSIX-compliant system)
    ...
    if (linux_ver)
    {
        ...
        if (linux_host)
        {
            ...
            if (linux_user)
            {
                printf("[+] User Name:            %s%s%s\n", COLOR_GREEN, linux_user,    COLOR_NC); free(linux_user);
                if (linux_model)   { printf("[+] Model:                %s%s%s\n", COLOR_GREEN, linux_model,   COLOR_NC); free(linux_model);
                    if (linux_arch)
                    {
                        if (linux_kernel)
                        {
                            linux_kernel[strcspn(linux_kernel, "\r\n")] = 0;
                            printf("[+] Architecture:         %s%s%s\n", COLOR_GREEN, linux_arch,    COLOR_NC); free(linux_arch);
                    ...
                    printf("[+] Kernel Version:       %s%s%s\n", COLOR_GREEN, linux_kernel, COLOR_NC);
                    free(linux_kernel);
                    if (linux_uptime) { printf("[+] Uptime:               %s%s%s\n", COLOR_GREEN, linux_uptime, COLOR_NC); free(linux_uptime); }
                    if (linux_load)   { printf("[+] Load Average:         %s%s%s\n", COLOR_GREEN, linux_load,   COLOR_NC); free(linux_load);   }
                    if (linux_cpu)    { printf("[+] CPU Count:            %s%s%s\n", COLOR_GREEN, linux_cpu,    COLOR_NC); free(linux_cpu);    }
                    if (linux_mem)    { printf("[+] Memory Size:          %s%s%s bytes\n", COLOR_GREEN, linux_mem,    COLOR_NC); free(linux_mem);    }
                    if (linux_swap)   { printf("[+] Swap Usage:           %s%s%s\n", COLOR_GREEN, linux_swap,   COLOR_NC); free(linux_swap);   }
                    if (linux_disk)   { printf("[+] Disk Space Available: %s%s%s\n", COLOR_GREEN, linux_disk,   COLOR_NC); free(linux_disk);   }
                    if (linux_temp)   { printf("[+] CPU Thermal Level:    %s%s%s\n", COLOR_GREEN, linux_temp,   COLOR_NC); free(linux_temp);   }
                    if (linux_battery){ printf("[+] Battery Percentage:   %s%s%s\n", COLOR_GREEN, linux_battery,COLOR_NC); free(linux_battery);}
                    if (linux_network){ printf("[+] Network IP Address:   %s%s%s\n", COLOR_GREEN, linux_network,COLOR_NC); free(linux_network);}
                    return;
                }
            }
        }
